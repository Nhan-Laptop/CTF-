...
def main():
    ...
    Server_Instance.register("admin", Server_Instance.rng.urandom(16))
    ...
    while True:
        ...
        if choice == '1':
            ...
            username = bytes.fromhex(input("Username: ").strip())
            password = bytes.fromhex(input("Password: ").strip())

            if Server_Instance.register(username, password):
                
                print("Registration successful, here is your account information: ")
                print(f"Username: {username.hex()}")
                print(f"ID: {hex(Server_Instance.ID[username])}")
            else:
            ...
            content = bytes.fromhex(input("Enter your message to admin: "))
